# 🤖 AgrrhBot — the ultimate anonymous-message Telegram bot

AgrrhBot (`@AgrrhBot`) is an advanced anonymous inbox bot. Every user gets a
...
- **Inline-mode sharing** — `@AgrrhBot` from *any* chat drops your link card.
...
- **Deep linking** — `t.me/AgrrhBot?start=<token>` opens a specific inbox.
...
1. Talk to [@BotFather](https://t.me/BotFather):
   - `/newbot` → choose a name and username (e.g. `AgrrhBot`).
   - Copy the **token** into `BOT_TOKEN` in `.env`.
   - `/setinline` → enable inline mode (powers the `@AgrrhBot` share card).
...
- **Seed it**: post your own `@AgrrhBot` link in your bio, Instagram/Twitter
