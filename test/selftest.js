 ...
 * no network. Validates the two-way threading, reactions, editing, settings,
 * wall and inline flows. Run with: npm run selftest
 */
...
function mockBot(username) {
  ...
  const bot = {
    ...
    async editMarkup() { return { ok: true }; },
...
(async () => {
  const bot = mockBot('AgrrhBot');
  const store = new Store();
  const cfg = { channelId: '@testchannel', globalPerHour: 80, pairPerHour: 40, minIntervalMs: 0 };
  ...
  assert(welcome && welcome.text.includes('AgrrhBot'), 'welcome mentions bot');
  assert(welcome.text.includes(`t.me/AgrrhBot?start=${alice.token}`), 'welcome contains deep link');
  ...
  // bob opens link again and sends; delivered should carry a spoiler entity
